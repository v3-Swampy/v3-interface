...
const Duration: React.FC<{ duration: durationPlugin.Duration; currentStakeDuration: number; onClick: (durationSeconds: number) => void }> = memo(
  ({ duration, currentStakeDuration, onClick }) => {
    const i18n = useI18n(transitions);
    ...
    return (
      <div
        ...
        onClick={() => onClick(duration.asSeconds())}
      >
        {value} {unit}
      </div>
    );
