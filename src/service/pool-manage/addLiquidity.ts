...
export const addLiquidity = async ({
  ...
  try {
    ...
    const tickLower = Unit.fromMinUnit(priceLower).equals(Zero) ? getMinTick(fee) : calcTickFromPrice({ price: Unit.fromMinUnit(priceLower), tokenA, tokenB });
    const tickUpper = priceUpper === 'NaN' ? getMaxTick(fee) : calcTickFromPrice({ price: Unit.fromMinUnit(priceUpper), tokenA, tokenB });
    // console.log(typeof tickLower);
