...
export const enhancePositionForUI = (position: Position, pool: Pool | null | undefined): PositionForUI => {
  ...
  const [amount0, amount1] = pool?.token0Price && liquidity ? calcAmountFromPrice({ liquidity, lower, current: pool.token0Price.mul(`1e${token0.decimals-token1.decimals}`), upper }) : [undefined, undefined];
