...
export const getUserFarmInfoOfPosition = async ({ position, pool }: { position: PositionForUI; pool: Pool }) => {
    ...
    params: [
      {
        ...
        data: UniswapV3Staker.func.interface.encodeFunctionData('multicall', [
          incentiveKeys.map((incentiveKey) => UniswapV3Staker.func.interface.encodeFunctionData('getStakeRewardInfo', [incentiveKey.key, position.id])),
        ]),
      },
      'latest',
    ],
  ...
  const hasLiquidityIncentiveKeys = stakeRewards.filter((item) => item.boostedLiquidity > 0n).map((item) => item.incentiveKey);
