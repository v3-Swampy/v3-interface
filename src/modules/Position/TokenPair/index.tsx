...
const TokenPair: React.FC<Props> = ({ position, leftToken, rightToken, inverted = false, showFee = true, className, symbolClassName, feeClassName, ...props }) => {
  ...
  return (
    <div className={cx('flex items-center text-14px', className)} {...props}>
      ...
      <span className={`mx-4px text-black-normal font-bold ${symbolClassName}`}>
        {leftTokenForUI?.symbol} / {rightTokenForUI?.symbol}
      </span>
      {showFee && (
        <span className={`inline-block px-8px h-20px leading-20px rounded-100px bg-orange-light text-center text-14px text-orange-normal font-medium ${feeClassName}`}>
          {fee / 10000}%
        </span>
      )}
    </div>
